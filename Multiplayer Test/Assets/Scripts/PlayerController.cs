    ...
    private void FixedUpdate() {
        ...
        if (!value) {
            ...
            if (down) {
                ...
                if (grabbed == null) {
                    ...
                    if (Physics.Raycast(transform.GetChild(0).position, Vector3.Normalize(transform.GetChild(0).forward), out hit2, 8, layerMask)) {
                        if (hit2.rigidbody != null) { grabbed = hit2.rigidbody; }
                    }
                }
            ...
            if (mousedown && grabbed != null) {
                grabbed.position = Lerp(grabbed.position,transform.GetChild(0).position + transform.GetChild(0).forward*6,0.125f/2);
            }
        }
